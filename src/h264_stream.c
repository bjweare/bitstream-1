...
MbPartPredMode_e I_slice_MbPartPredMode(int mb_type, int transform_size_8x8_flag)
{
...
MbPartPredMode_e MbPartPredMode(slice_data_t* mbl, int slice_type, int mb_type, int n)
{
	...
	int transform_size_8x8_flag = mbl->transform_size_8x8_flag;	//不存在时默认为0
	switch(slice_type)
	{
		case I_SLICE:
			if(!n)
			{
				ret = I_slice_MbPartPredMode(mb_type,transform_size_8x8_flag);
			}
			break;
		case SI_SLICE:
			if(!n)
		...
		case B_SLICE:
			...
			else if(mb_type >= 0 && mb_type <= 22)
			{
				if(!n)
				{
					if(mb_type == 1 || mb_type == 4 ||
						mb_type == 5 || mb_type == 8 ||
						mb_type == 9 || mb_type == 12 || mb_type == 13)
						ret = Pred_L0;
					else if(mb_type == 2 || mb_type == 6 ||
						mb_type == 7 || mb_type == 10 ||
						mb_type == 11 || mb_type == 14 || mb_type == 15)
						ret = Pred_L1;
					else if(mb_type == 3 || (mb_type >= 16 && mb_type <= 21))
						ret = BiPred;
					else if(mb_type == 0)
						ret = Direct;
				}
				...
				{
					if(mb_type == 4 || mb_type == 17 ||
						mb_type == 5 || mb_type == 10 ||
						mb_type == 11 || mb_type == 16 )
						ret = Pred_L0;
					else if(mb_type == 8 || mb_type == 6 ||
						mb_type == 7 || mb_type == 9 ||
						mb_type == 18 || mb_type == 19 )
						ret = Pred_L1;
					else if(mb_type == 20 || mb_type == 21 
						|| (mb_type >= 12 && mb_type <= 15))
						ret = BiPred;
				}
			}
			break;
...
int NumMbPart(int slice_type, int mb_type)
{
	int ret = 0;
	...
	switch(slice_type)
	{
		case P_SLICE:
		case SP_SLICE:
			if(mb_type == 0 || mb_type == 1 || mb_type == 3)
				ret = mb_type + 1;
			else if(mb_type == 2 || mb_type == 4)
				ret = mb_type;
			else
				ret = 1;
			break;
...
/***子宏块***/
MbPartPredMode_e SubMbPredMode(int sub_mb_type, int slice_type)
{
	MbPartPredMode_e ret = Na;
	...
	{
		if(sub_mb_type>=1 && sub_mb_type<=12)
		{
			if(sub_mb_type == 3 || sub_mb_type == 8 ||
				sub_mb_type == 9 || sub_mb_type == 12)
				return BiPred;
			else if(sub_mb_type & 0x01)	//odd
				return Pred_L1;
			else
				return Pred_L0;
		}
...
int NumSubMbPart(int sub_mb_type, int slice_type)
{
	int ret = -1;
	if(slice_type == P_SLICE)
	{
		...
		else if(sub_mb_type == 1 || sub_mb_type == 2)
			return sub_mb_type;
	}
	else if(slice_type == B_SLICE)
	{
		...
		else if(sub_mb_type>=4 && sub_mb_type<=9)
		{
			return 2;
		}
		else
...
int find_nal_unit(uint8_t* buf, int size, int* nal_start, int* nal_end)
{
    ...
    *nal_end = 0;
    ...
    {
        i++; // skip leading zero
        if (i+4 >= size)// did not find nal start
		{ 
			return 0; 
		} 
    }
    ...
    *nal_start = i;
        ...
        )
    {
        ...
        if (i+3 >= size) // did not find nal end, stream ended first
		{ 
			*nal_end = size; 
			return -1; 
		} 
...
/**
 ...
 @param[in,out] h          the stream object
 @param[in]     buf        the buffer
 @param[in]     size       the size of the buffer (size of NALU)
...
void read_seq_parameter_set_rbsp(h264_stream_t* h, bs_t* b)
{
    ...
    int seq_parameter_set_id = bs_read_ue(b, NULL);
    ...
    */
    ...
    {
        sps->chroma_format_idc = bs_read_ue(b, NULL);
        if( sps->chroma_format_idc == 3 )
        {
            sps->residual_colour_transform_flag = bs_read_u1(b);
        }
        sps->bit_depth_luma_minus8 = bs_read_ue(b, NULL);
        sps->bit_depth_chroma_minus8 = bs_read_ue(b, NULL);
    ...
    sps->log2_max_frame_num_minus4 = bs_read_ue(b, NULL);
    sps->pic_order_cnt_type = bs_read_ue(b, NULL);
    if( sps->pic_order_cnt_type == 0 )
    {
        sps->log2_max_pic_order_cnt_lsb_minus4 = bs_read_ue(b, NULL);
    }
    else if( sps->pic_order_cnt_type == 1 )
    {
        sps->delta_pic_order_always_zero_flag = bs_read_u1(b);
        sps->offset_for_non_ref_pic = bs_read_se(b, NULL);
        sps->offset_for_top_to_bottom_field = bs_read_se(b, NULL);
        sps->num_ref_frames_in_pic_order_cnt_cycle = bs_read_ue(b, NULL);
        for( i = 0; i < sps->num_ref_frames_in_pic_order_cnt_cycle; i++ )
        {
            sps->offset_for_ref_frame[ i ] = bs_read_se(b, NULL);
        }
    }
    sps->num_ref_frames = bs_read_ue(b, NULL);
    sps->gaps_in_frame_num_value_allowed_flag = bs_read_u1(b);
	
    sps->pic_width_in_mbs_minus1 = bs_read_ue(b, NULL);
	h->PicWidthInMbs = sps->pic_width_in_mbs_minus1 + 1;

	sps->pic_height_in_map_units_minus1 = bs_read_ue(b, NULL);
	h->PicHeightInMapUnits = sps->pic_height_in_map_units_minus1 + 1;
	
    sps->frame_mbs_only_flag = bs_read_u1(b);
	h->FrameHeightInMbs = ( 2 - sps->frame_mbs_only_flag ) * h->PicHeightInMapUnits;
    ...
    {
        sps->frame_crop_left_offset = bs_read_ue(b, NULL);
        sps->frame_crop_right_offset = bs_read_ue(b, NULL);
        sps->frame_crop_top_offset = bs_read_ue(b, NULL);
        sps->frame_crop_bottom_offset = bs_read_ue(b, NULL);
    }
...
void read_scaling_list(bs_t* b, int* scalingList, int sizeOfScalingList, int useDefaultScalingMatrixFlag )
{
    ...
    for( j = 0; j < sizeOfScalingList; j++ )
    {
        if( nextScale != 0 )
        {
            int delta_scale = bs_read_se(b, NULL);
...
void read_vui_parameters(h264_stream_t* h, bs_t* b)
{
    ...
    {
        sps->vui.chroma_sample_loc_type_top_field = bs_read_ue(b, NULL);
        sps->vui.chroma_sample_loc_type_bottom_field = bs_read_ue(b, NULL);
    }
    ...
    if( sps->vui.bitstream_restriction_flag )
    {
        ...
        sps->vui.max_bytes_per_pic_denom = bs_read_ue(b, NULL);
        sps->vui.max_bits_per_mb_denom = bs_read_ue(b, NULL);
        sps->vui.log2_max_mv_length_horizontal = bs_read_ue(b, NULL);
        sps->vui.log2_max_mv_length_vertical = bs_read_ue(b, NULL);
        sps->vui.num_reorder_frames = bs_read_ue(b, NULL);
        sps->vui.max_dec_frame_buffering = bs_read_ue(b, NULL);
    }
}
...
void read_hrd_parameters(h264_stream_t* h, bs_t* b)
{
    ...
    sps->hrd.cpb_cnt_minus1 = bs_read_ue(b, NULL);
    ...
    {
        sps->hrd.bit_rate_value_minus1[ SchedSelIdx ] = bs_read_ue(b, NULL);
        sps->hrd.cpb_size_value_minus1[ SchedSelIdx ] = bs_read_ue(b, NULL);
...
void read_pic_parameter_set_rbsp(h264_stream_t* h, bs_t* b)
{
    ...
    pps->seq_parameter_set_id = bs_read_ue(b, NULL);
    pps->entropy_coding_mode_flag = bs_read_u1(b);
    pps->pic_order_present_flag = bs_read_u1(b);
    pps->num_slice_groups_minus1 = bs_read_ue(b, NULL);

    if( pps->num_slice_groups_minus1 > 0 )
    {
        pps->slice_group_map_type = bs_read_ue(b, NULL);
        if( pps->slice_group_map_type == 0 )
        {
            for( i_group = 0; i_group <= pps->num_slice_groups_minus1; i_group++ )
            {
                pps->run_length_minus1[ i_group ] = bs_read_ue(b, NULL);
            }
        }
        else if( pps->slice_group_map_type == 2 )
        {
            for( i_group = 0; i_group < pps->num_slice_groups_minus1; i_group++ )
            {
                pps->top_left[ i_group ] = bs_read_ue(b, NULL);
                pps->bottom_right[ i_group ] = bs_read_ue(b, NULL);
            }
        }
                 ...
                 pps->slice_group_map_type == 5 )
        {
            ...
            pps->slice_group_change_rate_minus1 = bs_read_ue(b, NULL);
        }
        else if( pps->slice_group_map_type == 6 )
        {
            pps->pic_size_in_map_units_minus1 = bs_read_ue(b, NULL);
    ...
    pps->num_ref_idx_l0_active_minus1 = bs_read_ue(b, NULL);
    pps->num_ref_idx_l1_active_minus1 = bs_read_ue(b, NULL);
    pps->weighted_pred_flag = bs_read_u1(b);
    pps->weighted_bipred_idc = bs_read_u(b,2);
    pps->pic_init_qp_minus26 = bs_read_se(b, NULL);
    pps->pic_init_qs_minus26 = bs_read_se(b, NULL);
    pps->chroma_qp_index_offset = bs_read_se(b, NULL);
    ...
    if( pps->_more_rbsp_data_present )
    {
        ...
        pps->second_chroma_qp_index_offset = bs_read_se(b, NULL);
    }
    read_rbsp_trailing_bits(h, b);
}
...
{
	int usedbits = 0;
    usedbits += read_slice_header(h, b);
	printf("=====================slice header usedbits: %d\n",usedbits);
    ...
    /***条带数据语法***/
    read_slice_data(h, b); /* all categories of slice_data( ) syntax */
...
int read_slice_header(h264_stream_t* h, bs_t* b)
{
	int usedbits = 0;
    ...
    sh->first_mb_in_slice = bs_read_ue(b, &usedbits);
    sh->slice_type = bs_read_ue(b, &usedbits);
	if(sh->slice_type > 5)
		sh->slice_type -= 5;
    sh->pic_parameter_set_id = bs_read_ue(b, &usedbits);

    pps = h->pps = h->pps_table[sh->pic_parameter_set_id];
    sps = h->sps = h->sps_table[pps->seq_parameter_set_id];

	int tmp_v = sps->log2_max_frame_num_minus4 + 4;
	usedbits += tmp_v;
    sh->frame_num = bs_read_u(b, tmp_v ); // was u(v)
	...
	h->MbaffFrameFlag = (sps->mb_adaptive_frame_field_flag && !sh->field_pic_flag);	
	h->PicHeightInMbs = h->FrameHeightInMbs / ( 1 + sh->field_pic_flag );
	h->PicSizeInMbs = h->PicWidthInMbs * h->PicHeightInMbs;
	
    if( nal->nal_unit_type == 5 )
    {
        sh->idr_pic_id = bs_read_ue(b, &usedbits);
    }
    if( sps->pic_order_cnt_type == 0 )
    {
		tmp_v = sps->log2_max_pic_order_cnt_lsb_minus4 + 4;
		usedbits += tmp_v;
        sh->pic_order_cnt_lsb = bs_read_u(b, tmp_v); // was u(v)
        if( pps->pic_order_present_flag && !sh->field_pic_flag )
        {
            sh->delta_pic_order_cnt_bottom = bs_read_se(b, &usedbits);
        }
    }
    if( sps->pic_order_cnt_type == 1 && !sps->delta_pic_order_always_zero_flag )
    {
        sh->delta_pic_order_cnt[ 0 ] = bs_read_se(b, &usedbits);
        if( pps->pic_order_present_flag && !sh->field_pic_flag )
        {
            sh->delta_pic_order_cnt[ 1 ] = bs_read_se(b, &usedbits);
        }
    }
    if( pps->redundant_pic_cnt_present_flag )
    {
        sh->redundant_pic_cnt = bs_read_ue(b, &usedbits);
    }
    ...
    if( is_slice_type( sh->slice_type, SH_SLICE_TYPE_P ) || is_slice_type( sh->slice_type, SH_SLICE_TYPE_SP ) || is_slice_type( sh->slice_type, SH_SLICE_TYPE_B ) )
    {
        ...
        {
            sh->num_ref_idx_l0_active_minus1 = bs_read_ue(b, &usedbits); // FIXME does this modify the pps?
            if( is_slice_type( sh->slice_type, SH_SLICE_TYPE_B ) )
            {
                sh->num_ref_idx_l1_active_minus1 = bs_read_ue(b, &usedbits);
            }
        }
    }
    usedbits += read_ref_pic_list_reordering(h, b);
    ...
    {
        usedbits += read_pred_weight_table(h, b);
    }
    if( nal->nal_ref_idc != 0 )
    {
        usedbits += read_dec_ref_pic_marking(h, b);
    }
    if( pps->entropy_coding_mode_flag && ! is_slice_type( sh->slice_type, SH_SLICE_TYPE_I ) && ! is_slice_type( sh->slice_type, SH_SLICE_TYPE_SI ) )
    {
        sh->cabac_init_idc = bs_read_ue(b, &usedbits);
    }
    sh->slice_qp_delta = bs_read_se(b, &usedbits);
    if( is_slice_type( sh->slice_type, SH_SLICE_TYPE_SP ) || is_slice_type( sh->slice_type, SH_SLICE_TYPE_SI ) )
    {
        ...
        sh->slice_qs_delta = bs_read_se(b, &usedbits);
    }
    if( pps->deblocking_filter_control_present_flag )
    {
        sh->disable_deblocking_filter_idc = bs_read_ue(b, &usedbits);
        if( sh->disable_deblocking_filter_idc != 1 )
        {
            sh->slice_alpha_c0_offset_div2 = bs_read_se(b, &usedbits);
            sh->slice_beta_offset_div2 = bs_read_se(b, &usedbits);
        }
    }
    if( pps->num_slice_groups_minus1 > 0 &&
        pps->slice_group_map_type >= 3 && pps->slice_group_map_type <= 5)
    {
		tmp_v = intlog2( pps->pic_size_in_map_units_minus1 + pps->slice_group_change_rate_minus1 + 1 );
		usedbits += tmp_v;
        sh->slice_group_change_cycle = bs_read_u(b,  tmp_v); // was u(v) // FIXME add 2?
    }
    // bs_print_state(b);
...
int read_ref_pic_list_reordering(h264_stream_t* h, bs_t* b)
{
	int usedbits = 0;
    slice_header_t* sh = h->sh;
    ...
    if( ! is_slice_type( sh->slice_type, SH_SLICE_TYPE_I ) && ! is_slice_type( sh->slice_type, SH_SLICE_TYPE_SI ) )
    {
        ...
        if( sh->rplr.ref_pic_list_reordering_flag_l0 )
        {
            do
            {
                sh->rplr.reordering_of_pic_nums_idc = bs_read_ue(b, &usedbits);
                if( sh->rplr.reordering_of_pic_nums_idc == 0 ||
                    sh->rplr.reordering_of_pic_nums_idc == 1 )
                {
                    sh->rplr.abs_diff_pic_num_minus1 = bs_read_ue(b, &usedbits);
                }
                else if( sh->rplr.reordering_of_pic_nums_idc == 2 )
                {
                    sh->rplr.long_term_pic_num = bs_read_ue(b, &usedbits);
                }
...
int read_pred_weight_table(h264_stream_t* h, bs_t* b)
{
    ...
    sh->pwt.luma_log2_weight_denom = bs_read_ue(b, &usedbits);
    if( sps->chroma_format_idc != 0 )
    {
        sh->pwt.chroma_log2_weight_denom = bs_read_ue(b, &usedbits);
    }
    for( i = 0; i <= pps->num_ref_idx_l0_active_minus1; i++ )
    {
        ...
        {
            sh->pwt.luma_weight_l0[ i ] = bs_read_se(b, &usedbits);
            sh->pwt.luma_offset_l0[ i ] = bs_read_se(b, &usedbits);
        }
        if ( sps->chroma_format_idc != 0 )
        {
			usedbits += 1;
            ...
            if( sh->pwt.chroma_weight_l0_flag[i] )
            {
                for( j =0; j < 2; j++ )
                {
                    sh->pwt.chroma_weight_l0[ i ][ j ] = bs_read_se(b, &usedbits);
                    sh->pwt.chroma_offset_l0[ i ][ j ] = bs_read_se(b, &usedbits);
                }
            }
        }
    }
    if( is_slice_type( sh->slice_type, SH_SLICE_TYPE_B ) )
    {
        for( i = 0; i <= pps->num_ref_idx_l1_active_minus1; i++ )
        {
            ...
            {
                sh->pwt.luma_weight_l1[ i ] = bs_read_se(b, &usedbits);
                sh->pwt.luma_offset_l1[ i ] = bs_read_se(b, &usedbits);
            }
            if( sps->chroma_format_idc != 0 )
            {
				usedbits += 1;
                ...
                if( sh->pwt.chroma_weight_l1_flag[i] )
                {
                    for( j = 0; j < 2; j++ )
                    {
                        sh->pwt.chroma_weight_l1[ i ][ j ] = bs_read_se(b, &usedbits);
                        sh->pwt.chroma_offset_l1[ i ][ j ] = bs_read_se(b, &usedbits);
                    }
                }
            }
        }
...
int read_dec_ref_pic_marking(h264_stream_t* h, bs_t* b)
{
	int usedbits = 0;
    slice_header_t* sh = h->sh;
    // FIXME should be an array
...
int read_slice_data(h264_stream_t* h, bs_t* b)
{
	int usedbits = 0;
    slice_header_t* sh = h->sh;
    sps_t* sps = h->sps;
	...
	memset(sd, 0, sizeof(slice_data_t));
	
    if( pps->entropy_coding_mode_flag == CABAC )
        while( !bs_byte_aligned(b) )
        {
			usedbits += 1;
            sd->cabac_alignment_one_bit = bs_read_f(b,1);
        }
		
	int MbaffFrameFlag = h->MbaffFrameFlag;
    ...
    do {
        ...
        if( moreDataFlag ) 
		{
            ...
            {
				if(pps->entropy_coding_mode_flag == CAVLC)
					sd->mb_field_decoding_flag = bs_read_u1(b);
				else
					sd->mb_field_decoding_flag = 0; //bs_read_ae(v);
			}
        ...
        else 
		{
            ...
            if( MbaffFrameFlag && CurrMbAddr % 2 == 0 )
                ...
                sd->end_of_slice_flag = 0; //bs_read_ae(v);
...
int read_macroblock_layer(h264_stream_t* h, bs_t* b)
{
	int usedbits = 0;
	int i;
	...
	if(pps->entropy_coding_mode_flag == CAVLC)
		mb_type = bs_read_ue(b, &usedbits);	//ue(v)
	else
		mb_type = 0;	//ae(v)
	...
	{ 
		...
		if(sps->chroma_format_idc != 0)	//非单色 
		{
			//表6-1
	...
	{ 
		...
		{ 
			/***子宏块预测语法***/		
			if(pps->entropy_coding_mode_flag == CAVLC)
				read_sub_mb_pred_cavlc(h, b, mb_type);
			else
		...
		{ 
			...
			/***宏块预测语法***/
			if(pps->entropy_coding_mode_flag == CAVLC)
				read_mb_pred_cavlc(h, b, mb_type);
			else
				;//read_mb_pred_cabac();
		} 
...
void read_mb_pred_cavlc(h264_stream_t* h, bs_t* b,int mb_type) 
{
	int usedbits = 0;
...
void read_sub_mb_pred_cavlc(h264_stream_t* h, bs_t* b, int mb_type) 
{
	int usedbits = 0;
...
#if 0
void read_mb_pred_cabac( mb_type ) 
{
		...
		|| MbPartPredMode( mb_type, 0 ) == Intra_16x16 ) 
	{
		if(  MbPartPredMode( mb_type, 0 ) == Intra_4x4 )
			...
			{
				prev_intra4x4_pred_mode_flag[ luma4x4BlkIdx ]	//ae(v)
				
				if( !prev_intra4x4_pred_mode_flag[ luma4x4BlkIdx ] )
					rem_intra4x4_pred_mode[ luma4x4BlkIdx ]		//ae(v)
			}
			
		if( MbPartPredMode( mb_type, 0 ) == Intra_8x8 )
			for( luma8x8BlkIdx=0; luma8x8BlkIdx<4; luma8x8BlkIdx++ ) 
			{
				prev_intra8x8_pred_mode_flag[ luma8x8BlkIdx ] 	//ae(v)
				
				if( !prev_intra8x8_pred_mode_flag[luma8x8BlkIdx])
					rem_intra8x8_pred_mode[ luma8x8BlkIdx ] 	//ae(v)
			}
		
		if( ChromaArrayType == 1 || ChromaArrayType == 2 )
			intra_chroma_pred_mode	// 2 ae(v)
	} 
	else if( MbPartPredMode( mb_type, 0 ) != Direct ) 
	{
		...
		for( mbPartIdx = 0; mbPartIdx < NumMbPart( mb_type ); mbPartIdx++)
			if( ( num_ref_idx_l0_active_minus1 > 0 || mb_field_decoding_flag != field_pic_flag ) 
				...
				ref_idx_l0[ mbPartIdx ] // 2 ae(v)
			
		for( mbPartIdx = 0; mbPartIdx < NumMbPart( mb_type ); mbPartIdx++)
			if( ( num_ref_idx_l1_active_minus1 > 0 || mb_field_decoding_flag != field_pic_flag ) 
				&& MbPartPredMode( mb_type, mbPartIdx ) != Pred_L0 )
				ref_idx_l1[ mbPartIdx ] // 2 ae(v)
				
		for( mbPartIdx = 0; mbPartIdx < NumMbPart( mb_type ); mbPartIdx++)
			if( MbPartPredMode ( mb_type, mbPartIdx ) != Pred_L1 )
				for( compIdx = 0; compIdx < 2; compIdx++ )
					mvd_l0[ mbPartIdx ][ 0 ][ compIdx ]  // 2 ae(v)
					
		for( mbPartIdx = 0; mbPartIdx < NumMbPart( mb_type ); mbPartIdx++)
			if( MbPartPredMode( mb_type, mbPartIdx ) != Pred_L0 )
				for( compIdx = 0; compIdx < 2; compIdx++ )
					mvd_l1[ mbPartIdx ][ 0 ][ compIdx ]  // 2 ae(v)
	}
}

void read_sub_mb_pred_cabac( mb_type ) 
{
	...
	for( mbPartIdx = 0; mbPartIdx < 4; mbPartIdx++ )
		if( ( num_ref_idx_l0_active_minus1 > 0 || mb_field_decoding_flag != field_pic_flag ) 
			&& mb_type != P_8x8ref0 && sub_mb_type[ mbPartIdx ] != B_Direct_8x8 
	...
	for( mbPartIdx = 0; mbPartIdx < 4; mbPartIdx++ )
		if( sub_mb_type[ mbPartIdx ] != B_Direct_8x8 
			...
			for( subMbPartIdx = 0; subMbPartIdx < NumSubMbPart(sub_mb_type[mbPartIdx]); subMbPartIdx++)
				for( compIdx = 0; compIdx < 2; compIdx++ )
					mvd_l0[ mbPartIdx ][ subMbPartIdx ][ compIdx ] // 2	ae(v)
	...
	for( mbPartIdx = 0; mbPartIdx < 4; mbPartIdx++ )
		if( sub_mb_type[ mbPartIdx ] != B_Direct_8x8 
			...
			for( subMbPartIdx = 0; subMbPartIdx < NumSubMbPart( sub_mb_type[ mbPartIdx ] ); subMbPartIdx++)
				for( compIdx = 0; compIdx < 2; compIdx++ )
					mvd_l1[ mbPartIdx ][ subMbPartIdx ][ compIdx ] // 2	ae(v)
}
#endif
