...
static uint32_t bs_read_ue(bs_t* b, int* usedbits);
static int32_t  bs_read_se(bs_t* b, int* usedbits);
...
//byte_aligned( )比特流中的下一比特是字节第一个比特，返回值为TRUE
...
//ue(v)
static inline uint32_t bs_read_ue(bs_t* b, int* usedbits)
{
    int32_t r = 0;	//codeNum
...
//se(v)
static inline int32_t bs_read_se(bs_t* b, int* usedbits) 
{
    int32_t r = bs_read_ue(b,usedbits);
    if (r & 0x01)	//odd
    {
        r = (r+1)/2;
    }
